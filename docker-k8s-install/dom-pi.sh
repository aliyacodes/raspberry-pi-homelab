...
# Enable cgroups
sudo sed -i 's/$/ cgroup_enable=cpuset cgroup_enable=memory cgroup_memory=1/' /boot/firmware/cmdline.txt #append "swapaccount=1"?
...
# After joining nodes with token, check status on dom-pi with "kubectl get nodes"